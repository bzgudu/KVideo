...
function getEnvSubscriptions(customValue?: string): SourceSubscription[] {
  const envValue = (customValue || process.env.SUBSCRIPTION_SOURCES || process.env.NEXT_PUBLIC_SUBSCRIPTION_SOURCES || 'https://raw.githubusercontent.com/rapier15sapper/ew/refs/heads/main/test.json').trim();
  if (!envValue) return [];
